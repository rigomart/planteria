...
import { requireActionOwnership, requireDeliverableOwnership } from "./lib/ownership";
...
export const addAction = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const maxOrder =
      existingActions.length > 0 ? Math.max(...existingActions.map((a) => a.order)) : -1;
...
export const updateAction = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { action, outcome } = await requireActionOwnership(ctx, args.actionId, identity.subject);
...
export const listByDeliverable = query({
  ...
  handler: async (ctx, args) => {
    ...
    await requireDeliverableOwnership(ctx, args.deliverableId, identity.subject);

    const actions = await ctx.db
      .query("actions")
      .withIndex("by_deliverable_order", (q) => q.eq("deliverableId", args.deliverableId))
      .collect();
