...
function validateApiKey(value: string) {
  ...
  if (!API_KEY_PATTERN.test(trimmed)) {
    throw new Error("OpenAI API key must start with sk- and be at least 24 characters");
  }
...
export const generatePlanteriaApiKey = mutation({
  ...
  handler: async (ctx) => {
    ...
    const existing = await findKeyDoc(ctx, identity.subject, PROVIDERS.PLANTERIA);
