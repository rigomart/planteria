...
export const addDeliverable = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const maxOrder =
      existingDeliverables.length > 0 ? Math.max(...existingDeliverables.map((d) => d.order)) : -1;
...
export const updateDeliverable = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { outcome } = await requireOutcomeOwnership(ctx, deliverable.outcomeId, identity.subject);
