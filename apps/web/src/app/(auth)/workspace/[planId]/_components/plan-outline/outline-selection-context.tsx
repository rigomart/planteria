...
const OutlineSelectionContext = createContext<OutlineSelectionContextValue | null>(null);

export function OutlineSelectionProvider({ children }: { children: React.ReactNode }) {
  ...
  const selectDeliverable = useCallback((outcomeId: string, deliverableId: string) => {
  ...
  return (
    <OutlineSelectionContext.Provider value={value}>{children}</OutlineSelectionContext.Provider>
  );
}
...
export function useOutlineSelection() {
  ...
  if (!context) {
    throw new Error("useOutlineSelection must be used within an OutlineSelectionProvider");
  }
