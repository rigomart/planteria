...
type PlanPreviewData = FunctionReturnType<typeof api.plans.queries.getPlanPreview>;
...
function PlanWorkspacePreviewContent({ plan }: PlanWorkspacePreviewContentProps) {
  ...
  return (
    ...
    >
      ...
      <div className={cn("overflow-hidden rounded-lg border bg-background flex-1")}>
        <div className="flex h-full flex-col">
          {isLoading ? (
            ...
            <>
              ...
              <TabsContent value="markdown" className="flex-1 overflow-auto px-6 pb-10 pt-6">
                {hasContent ? (
...
function buildPlanMarkdown(preview: PlanPreviewData): string {
  ...
  outcomes.forEach((outcome, outcomeIndex) => {
    ...
    const deliverableBlocks = outcome.deliverables.map((deliverable, deliverableIndex) => {
      const deliverableTitle = deliverable.title?.trim() || `Deliverable ${deliverableIndex + 1}`;
      ...
      if (deliverable.actions.length === 0) {
        ...
        deliverable.actions.forEach((action, actionIndex) => {
          const actionTitle = action.title?.trim() || `Action ${actionIndex + 1}`;
          details.push(`  - ${checkboxForStatus(action.status)} ${actionTitle}`);
        });
      }
...
const markdownComponents: MarkdownComponents = {
  ...
  h2: ({ className, ...props }) => (
    <h2
      className={cn("mt-8 text-2xl font-semibold text-foreground first:mt-0", className)}
      {...props}
    />
  ),
  h3: ({ className, ...props }) => (
    <h3
      className={cn("mt-6 text-xl font-medium text-foreground first:mt-0", className)}
      {...props}
    />
  ),
  p: ({ className, ...props }) => (
  ...
  li: ({ className, ...props }) => (
    <li className={cn("leading-6 text-muted-foreground", className)} {...props} />
  ),
