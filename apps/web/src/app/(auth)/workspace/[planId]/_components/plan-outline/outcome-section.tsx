...
export function OutcomeSection({ planId, outcome, index }: OutcomeSectionProps) {
  ...
  const updateOutcome = useMutation(api.outcomes.updateOutcome).withOptimisticUpdate(
    (localStore, args) => {
      ...
      if (outcomes !== undefined) {
        ...
        localStore.setQuery(api.outcomes.queries.listByPlan, { planId }, nextOutcomes);
      }
    },
  );

  const deleteOutcome = useMutation(api.outcomes.deleteOutcome).withOptimisticUpdate(
    (localStore, args) => {
      const outcomes = localStore.getQuery(api.outcomes.queries.listByPlan, {
        planId,
      });
      ...
      if (outcomes !== undefined) {
        ...
        localStore.setQuery(api.outcomes.queries.listByPlan, { planId }, remaining);
      }
    },
  );

  const addDeliverable = useMutation(api.deliverables.addDeliverable).withOptimisticUpdate(
    (localStore, args) => {
      const current = localStore.getQuery(api.deliverables.queries.listByOutcome, {
      ...
      if (current !== undefined) {
        ...
        localStore.setQuery(api.deliverables.queries.listByOutcome, { outcomeId: args.outcomeId }, [
          ...current,
          tempDeliverable,
        ]);
      }
  ...
  const completedDeliverables =
    deliverables?.reduce((count, item) => (item.status === "done" ? count + 1 : count), 0) ?? 0;
